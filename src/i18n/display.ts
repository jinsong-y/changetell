import type { TrigramName } from '../utils/iching-data';
...
export const STRENGTH_EN: Record<string, string> = {
  旺: 'Prosperous',
  相: 'Supported',
  囚: 'Confined',
  休: 'Resting',
...
export const display = (locale: 'zh-CN' | 'en', value: string) => {
  if (locale === 'zh-CN') return value;
  return displayEnglish(value) ?? value;
};

export const displayRequired = (locale: 'zh-CN' | 'en', value: string, category: string) => {
  if (locale === 'zh-CN') return value;
